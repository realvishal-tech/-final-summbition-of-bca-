...
const ADMIN_TOP_NAV_META = {
  dashboard: { title: 'Dashboard', subtitle: 'A calm control center for content, attendance, and student growth.' },
  ...
  attendance: { title: 'Attendance', subtitle: 'Start or review live sessions and export reports.' },
    ...
    <div class="mobile-drawer" id="mobileDrawer">
      ...
      <div class="drawer-nav">
        ...
        <div class="drawer-grid-menu">
          ...
          <a href="elite.html" class="drawer-grid-item ${currentPage.includes('elite.html') ? 'active' : ''}" style="border: 1px dashed rgba(192, 132, 252, 0.4); background: rgba(124, 58, 237, 0.05);">
            <div class="grid-item-icon">💎</div>
            <div class="grid-item-text" style="color: #c084fc; font-weight: 700;">ELITE+</div>
          </a>
  ...
  setTimeout(() => {
    ...
    if (emailOrUser === ve.toLowerCase() && pass === vp) {
      ...
      const loginRouteTab = (() => {
        ...
        return (queryTab || hashTab || lsGet('lastAdminTab', 'dashboard')).trim().toLowerCase();
      })();
      switchAdminTab(loginRouteTab || 'dashboard');
...
function initGateTerminal() {
  const el = document.getElementById('wgTermText');
  if (!el) return;
  const messages = [
    "INITIALIZING SECURE CONNECTION...",
    "ESTABLISHING HANDSHAKE PROTOCOL...",
    "ACCESSING BCA MAINFRAME...",
    "BYPASSING FIREWALL...",
    "DECRYPTING STUDENT DATABASE...",
    "SYSTEM READY. PLEASE IDENTIFY."
  ];
  let msgIdx = 0;
  let charIdx = 0;
  let typingTimeout;
  ...
  function type() {
    ...
    if (charIdx < messages[msgIdx].length) {
      el.textContent += messages[msgIdx].charAt(charIdx);
      charIdx++;
      typingTimeout = setTimeout(type, 30 + Math.random() * 50);
    } else {
  ...
  // Clear any existing timeout if re-initializing
...
// ============================================================
// PREMIUM YOUTUBE + NATIVE VIDEO POPUP PLAYER
// ============================================================
...
function closeVideoPlayer() {
  // Video player popup completely removed
}
...
// Automatically inject Elite Zone button into desktop navigation bar
document.addEventListener('DOMContentLoaded', () => {
  const navLinks = document.querySelector('.nav-links');
  if (navLinks && !navLinks.querySelector('a[href="elite.html"]')) {
    const eliteLink = document.createElement('a');
    eliteLink.href = 'elite.html';
    eliteLink.className = 'nav-link elite-nav-btn';
    eliteLink.innerHTML = '💎 ELITE+';

    if (window.location.pathname.includes('elite.html')) {
      eliteLink.classList.add('active');
    }

    // Insert after "My Batch" link if present
    const batchLink = navLinks.querySelector('a[href="my-batches.html"]');
    if (batchLink) {
      batchLink.after(eliteLink);
    } else {
      const firstLink = navLinks.querySelector('a');
      if (firstLink) {
        firstLink.after(eliteLink);
      } else {
        navLinks.appendChild(eliteLink);
      }
    }
  }
});
